...
export default (): PriceCalcStore => {
  ...
  const craftModule = createMemo(() =>
    state.simpleMode ? 0 : get.craftModule(variantId())
  );
  const craftAmmount = createMemo(() =>
    state.simpleMode ? 1 : get.craftAmmount(variantId())
  );
  const craftLavish = createMemo(() =>
    state.simpleMode ? false : get.craftLavish(variantId())
  );
  const craftLevel = createMemo(() =>
    state.simpleMode ? 1 : get.craftLevel(recipeSkill())
  );
  const recipeMargin = createMemo(() =>
    state.simpleMode ? 10 : get.recipeMargin(variantId())
  );
    ...
    return variant.Variant.Ingredients.map((ingredient) => {
      ...
      return {
        ...
        unitPrice: calcPrice(
          ...
          state.simpleMode
            ? allItemsAndTagsWithPrice()?.[getIngredientId(ingredient)]
                ?.AvgPrice ?? 0
  ...
  const tableJouleConsumption = createMemo(
    () => TableWatts[recipe()?.CraftingTable ?? ''] ?? 0
  );
  ...
  const calcExplanation = (productName: string, currencyName: string) => {
    const ingredients = recipeIngredients();
    const ingredientsStr = ingredients.map(
      (ingredient) =>
        ` - ${ingredient.calcQuantity}(${
          craftAmmount() === 1 ? '' : craftAmmount() + 'x'
        }${ingredient.Ammount}${
          !ingredient.IsStatic ? 'x' + moduleReductions[craftModule()] : ''
        }${
          !ingredient.IsStatic && craftLavish() ? 'x0.95' : ''
        }) ${getIngredientDisplayName(ingredient)} @ ${get.personalPrice(
          getIngredientId(ingredient)
        )} ${currencyName} = ${ingredient.calcPrice} ${currencyName}`
    );
    const product = recipeProducts().find(
      (product) => product.Name === productName
    );
    return [
      `Calculated cost of ${productName} = ${product?.productionCost} ${currencyName}`,
      `Using recipe ${focusedNode()?.selectedVariant?.Variant.Key} at table ${
        recipe()?.CraftingTable
      } with module lvl ${craftModule()}`,
      `From ${product?.Ammount !== 1 ? `1/${product?.Ammount} of ` : ''}${
        product?.costPercentage !== 100 ? `${product?.costPercentage}% of ` : ''
      }${formatNumber(
        totalIngredientCost() / craftAmmount()
      )} ${currencyName} ${
        craftAmmount() > 1 ? `(total of ${totalIngredientCost()} each) ` : ''
      }from the sum of ingredients for ${craftAmmount()} repetition(s):`,
      ...ingredientsStr,
      recipeCalorieTotalCost() > 0
        ...
        : undefined,
      recipeJoules() > 0
        ? ` - Fuel ${formatNumber(recipeFuelTotalCost())} ${currencyName}`
        : undefined,
    ]
      .filter((t) => !!t)
      .join('\n');
  ...
  return {
    ...
    update: {
      toggleSimpleMode: () =>
        setState((prev) => ({ simpleMode: !prev.simpleMode })),
