...
export default (props: Props) => {
  const { allProductsInStores } = useMainContext();
  const allProductsInStore = createMemo(() =>
    allProductsInStores()?.filter((t) => t.StoreName === props.storeName)
  );
  const allProductsForSaleInStore = createMemo(() =>
    allProductsInStore()?.filter((t) => !t.Buying)
  );
  const allProductsForBuyInStore = createMemo(() =>
    allProductsInStore()?.filter((t) => t.Buying)
  );
  return (
    <>
